	...
	if ( $f_send_email_notification ) {
		...
		if ( !empty( $t_changes ) ) {
			...
			if( null === email_store( $t_email, $t_subject, $t_message ) ) {
				log_event( LOG_EMAIL, sprintf( 'Notification envoy&eacute;e &agrave; ' . $f_username ) );
			} else {
				log_event( LOG_EMAIL, sprintf( 'Notification envoy&eacute;e &agrave; ' . $f_username . ' (' . $t_email . ')' ) );
