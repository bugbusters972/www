...
<table class="acc_box" cellspacing="1">
<?php
# -- Navigation header row --?>
<tr>
<?php
# -- Viewing range info --?>
	<td class="form-title" width="386" colspan="2">
<?php
...
# -- Loop over bug rows and create $v_* variables --
	...
	$t_nom = string_display_line_links( $t_bug->nom );
...
<tr bgcolor="<?php echo $status_color?>">
	...
	if( !bug_is_readonly( $t_bug->id ) && access_has_bug_level( $t_update_bug_threshold, $t_bug->id ) ) {
		echo '<a href="' . string_get_bug_update_url( $t_bug->id ) . '"><img border="0" src="' . $t_icon_path . 'update.png' . '" alt="' . lang_get( 'update_bug_button' ) . '" title="Modifier le dossier"/></a>';
	}

	if( ON == config_get( 'show_motif_nvvenue_text' ) ) {
		print_formatted_motif_nvvenue_string( $t_bug );
	} else {
		print_status_icon( $t_bug->motif_nvvenue );
	}
	...
	<td class="left" valign="top" width="100%">
		...
		<?php
			...
			echo '<a href="view.php?id='.$t_bug->id.'">'.string_display_line($t_nom).'</a>';
	?>
		<br />
		<?php
	...
	if( $t_bug->last_updated > strtotime( '-' . $t_filter[FILTER_PROPERTY_HIGHLIGHT_CHANGED] . ' heures' ) ) {
