...
function bugnote_add($f_bes_com,$f_bes_soc,$f_bes_vio,$f_nat_vio_cpl,$f_enf_temoin_cpl,$f_enf_vio_cpl,$f_comm_cpl,$f_cond_trav,$f_isol_trav,$f_dign_trav,$f_vio_trav,$f_juri_trav,$f_domaine,$f_nom_vio,$f_prenom_vio,$f_age_vio,$f_sexe_vio,$f_adresse_vio,$f_emploi_vio,$f_ressources_vio,$f_secteur_vio,$f_csp_vio,$f_lien_vict_vio,$f_vict_enf_vio,$f_mere_vict_vio,$f_pb_vio,$f_comm_vio,$f_cons_phys,$f_cons_psy,$f_cons_trav,$f_cons_soc,$f_cons_comm,$f_dur_violence, $f_lieu_violence, $p_bug_id, $p_time_tracking = '0:00', $p_private = false, $p_type = BUGNOTE, $p_attr = '', $p_user_id = null, $p_send_email = TRUE, $p_log_history = TRUE) {

	$c_bug_id = db_prepare_int( $p_bug_id );
	$c_time_tracking = helper_duration_to_minutes( $p_time_tracking );
	$c_type = db_prepare_int( $p_type );
	$c_dur_violence = db_prepare_string( $f_dur_violence );
	$c_lieu_violence = db_prepare_string( $f_lieu_violence );
	$c_nat_vio_cpl =  db_prepare_string($f_nat_vio_cpl);
	$c_enf_temoin_cpl =  db_prepare_int($f_enf_temoin_cpl);
	$c_enf_vio_cpl =  db_prepare_int($f_enf_vio_cpl);
	$c_comm_cpl =  db_prepare_string($f_comm_cpl);
	$c_cond_trav =  db_prepare_string($f_cond_trav);
	$c_isol_trav =  db_prepare_string($f_isol_trav);
	$c_dign_trav=  db_prepare_string($f_dign_trav);
	$c_vio_trav=  db_prepare_string($f_vio_trav);
	$c_juri_trav=  db_prepare_string($f_juri_trav);
	$c_domaine=  db_prepare_int($f_domaine);
	$c_nom_vio=  db_prepare_string($f_nom_vio);
	$c_prenom_vio=  db_prepare_string($f_prenom_vio);
	$c_age_vio=  db_prepare_int($f_age_vio);
	$c_sexe_vio=  db_prepare_int($f_sexe_vio);
	$c_adresse_vio=  db_prepare_int($f_adresse_vio);
	$c_emploi_vio=  db_prepare_string($f_emploi_vio);
	$c_ressources_vio=  db_prepare_int($f_ressources_vio);
	$c_secteur_vio=  db_prepare_int($f_secteur_vio);
	$c_csp_vio=  db_prepare_int($f_csp_vio);
	$c_lien_vict_vio=  db_prepare_int($f_lien_vict_vio);
	$c_vict_enf_vio=  db_prepare_int($f_vict_enf_vio);
	$c_mere_vict_vio=  db_prepare_int($f_mere_vict_vio);
	$c_pb_vio=  db_prepare_string($f_pb_vio);
	$c_comm_vio=  db_prepare_string($f_comm_vio);
	$c_cons_phys=  db_prepare_string($f_cons_phys);
	$c_cons_psy=  db_prepare_string($f_cons_psy);
	$c_cons_trav=  db_prepare_string($f_cons_trav);
	$c_cons_soc=  db_prepare_string($f_cons_soc);
	$c_cons_comm=  db_prepare_string($f_cons_comm);
	$c_bes_vio=  db_prepare_string($f_bes_vio);
	$c_bes_soc=  db_prepare_string($f_bes_soc);
	$c_bes_com=  db_prepare_string($f_bes_com);
	...
	if( REMINDER !== $p_type ) {
		...
		if( ON == $t_time_tracking_enabled && $c_time_tracking > 0 ) {
			...
			if( is_blank( $c_dur_violence ) && OFF == $t_time_tracking_without_note ) {
				error_parameters( lang_get( 'bugnote' ) );
				trigger_error( ERROR_EMPTY_FIELD, ERROR );
			}
			$c_type = TIME_TRACKING;
		} else if( is_blank( $c_dur_violence ) ) {
	...
	# Event integration
$c_dur_violence = event_signal( 'EVENT_BUGNOTE_DATA', $f_dur_violence , $c_bug_id );
$c_lieu_violence = event_signal( 'EVENT_BUGNOTE_DATA', $f_lieu_violence , $c_bug_id );
$c_nat_vio_cpl =  event_signal( 'EVENT_BUGNOTE_DATA',$f_nat_vio_cpl, $c_bug_id );
$c_enf_temoin_cpl =  event_signal( 'EVENT_BUGNOTE_DATA',$f_enf_temoin_cpl, $c_bug_id );
$c_enf_vio_cpl =  event_signal( 'EVENT_BUGNOTE_DATA',$f_enf_vio_cpl, $c_bug_id );
$c_comm_cpl =  event_signal( 'EVENT_BUGNOTE_DATA',$f_comm_cpl, $c_bug_id );
$c_cond_trav =  event_signal( 'EVENT_BUGNOTE_DATA',$f_cond_trav, $c_bug_id );
$c_isol_trav =  event_signal( 'EVENT_BUGNOTE_DATA',$f_isol_trav, $c_bug_id );
$c_dign_trav=  event_signal( 'EVENT_BUGNOTE_DATA',$f_dign_trav, $c_bug_id );
$c_vio_trav=  event_signal( 'EVENT_BUGNOTE_DATA',$f_vio_trav, $c_bug_id );
$c_juri_trav=  event_signal( 'EVENT_BUGNOTE_DATA',$f_juri_trav, $c_bug_id );
$c_domaine=  event_signal( 'EVENT_BUGNOTE_DATA',$f_domaine, $c_bug_id );
$c_nom_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_nom_vio, $c_bug_id );
$c_prenom_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_prenom_vio, $c_bug_id );
$c_age_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_age_vio, $c_bug_id );
$c_sexe_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_sexe_vio, $c_bug_id );
$c_adresse_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_adresse_vio, $c_bug_id );
$c_emploi_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_emploi_vio, $c_bug_id );
$c_ressources_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_ressources_vio, $c_bug_id );
$c_secteur_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_secteur_vio, $c_bug_id );
$c_csp_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_csp_vio, $c_bug_id );
$c_lien_vict_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_lien_vict_vio, $c_bug_id );
$c_vict_enf_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_vict_enf_vio, $c_bug_id );
$c_mere_vict_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_mere_vict_vio, $c_bug_id );
$c_pb_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_pb_vio, $c_bug_id );
$c_comm_vio=  event_signal( 'EVENT_BUGNOTE_DATA',$f_comm_vio, $c_bug_id );
$c_cons_phys=  event_signal( 'EVENT_BUGNOTE_DATA',$f_cons_phys, $c_bug_id );
$c_cons_psy=  event_signal( 'EVENT_BUGNOTE_DATA',$f_cons_psy, $c_bug_id );
$c_cons_trav=  event_signal( 'EVENT_BUGNOTE_DATA',$f_cons_trav, $c_bug_id );
$c_cons_soc=  event_signal( 'EVENT_BUGNOTE_DATA',$f_cons_soc, $c_bug_id );
$c_cons_comm=  event_signal( 'EVENT_BUGNOTE_DATA',$f_cons_comm, $c_bug_id );	


	# insert bugnote text
	$query = 'INSERT INTO ' . $t_bugnote_text_table . ' ( bes_com, bes_soc, bes_vio, dur_violence, lieu_violence, nat_vio_cpl, enf_temoin_cpl, enf_vio_cpl, comm_cpl, cond_trav, isol_trav, dign_trav, vio_trav, juri_trav, domaine, nom_vio, prenom_vio, age_vio, sexe_vio, adresse_vio, emploi_vio, ressources_vio, secteur_vio, csp_vio, lien_vict_vio, vict_enf_vio, mere_vict_vio, pb_vio, comm_vio, cons_phys, cons_psy, cons_trav, cons_soc, cons_comm ) VALUES ( ' . db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() .' )';
	
	db_query_bound( $query, Array( $c_bes_com, $c_bes_soc, $c_bes_vio, $c_dur_violence ,$c_lieu_violence ,$c_nat_vio_cpl ,$c_enf_temoin_cpl ,$c_enf_vio_cpl ,$c_comm_cpl ,$c_cond_trav ,$c_isol_trav ,$c_dign_trav,$c_vio_trav,$c_juri_trav,$c_domaine,$c_nom_vio,$c_prenom_vio,$c_age_vio,$c_sexe_vio,$c_adresse_vio,$c_emploi_vio,$c_ressources_vio,$c_secteur_vio,$c_csp_vio,$c_lien_vict_vio,$c_vict_enf_vio,$c_mere_vict_vio,$c_pb_vio,$c_comm_vio,$c_cons_phys,$c_cons_psy,$c_cons_trav,$c_cons_soc,$c_cons_comm ) );
	...
	if( TRUE == $p_send_email && !is_blank( $c_dur_violence ) ) {
...
function bugnote_delete( $p_bugnote_id ) {
	...
	# Remove the bugnote
	$query = 'DELETE FROM ' . $t_bugnote_table . ' WHERE bugnote_text_id=' . db_param();
...
#delete reponse
function reponse_delete( $p_reponse_id ) {
	$c_reponse_id = db_prepare_int( $p_reponse_id );
	#$t_bug_id = bugnote_get_field( $p_reponse_id, 'bug_id' );
	#$t_bugnote_text_id = bugnote_get_field( $p_reponse_id, 'bugnote_text_id' );
	#$t_bugnote_text_table = db_get_table( 'mantis_bugnote_text_table' );
	$t_reponse_table = db_get_table( 'bugnote_reponse_table' );

	# Remove the bugnote
	$query = 'DELETE FROM ' . $t_reponse_table . ' WHERE id=' . db_param();
	db_query_bound( $query, Array( $c_reponse_id ) );
	...
	# log deletion of bug
	#history_log_event_special( $t_bug_id, BUGNOTE_DELETED, bugnote_format_id( $p_reponse_id ) );
...
function bugnote_delete_all( $p_bug_id ) {
	...
	$querie	= "DELETE FROM $t_reponse_table
			          	WHERE bugnote_id=" . db_param();
		db_query_bound( $querie, Array( $t_bugnote_text_id ) );
	}
 ...
 * @access public
 */
function bugnote_recup_field( $p_bugnote_id,$champ ) {
	#$t_bugnote_text_id = bugnote_get_field( $p_bugnote_id, 'bugnote_text_id' );
	$t_bugnote_text_table = db_get_table( 'mantis_bugnote_text_table' );
...
function reponse_recup_field( $p_reponse_id,$champ ) {
	$t_reponse_table = db_get_table( 'bugnote_reponse_table' );
	...
	$query = "SELECT ".$champ."
		          	...
		          	WHERE id=" . db_param();
	$result = db_query_bound( $query, Array( $p_reponse_id ) );
...
function bugnote_get_field( $p_bugnote_id, $p_field_name ) {
	...
	$query = "SELECT $c_field_name
		          	FROM $t_bugnote_table
		          	WHERE bugnote_text_id=" . db_param();
...
function bugnote_get_all_bugnotes( $p_bug_id ) {
	...
	if( !isset( $g_cache_bugnotes[(int)$p_bug_id] ) ) {
		...
		#foreach ($t_result as $that) {foreach ($that as $minithat){ echo $minithat.'__';}};
 ...
 * @access public
 */
function bugnote_set_field( $p_bugnote_id, $champ ) {

$lechamp = $GLOBALS['f_'.$champ];
$t_old_field = bugnote_recup_field( $p_bugnote_id, $champ );
if ($lechamp!=null){

	if ( $t_old_field == $lechamp ) {
		return true;
	}
	...
	if ( bug_revision_count( $t_bug_id, REV_BUGNOTE, $p_bugnote_id ) < 1 ) {
		...
		bug_revision_add( $t_bug_id, $t_user_id, REV_BUGNOTE, $t_old_field, $p_bugnote_id, $t_timestamp );
	}

	$query = "UPDATE $t_bugnote_text_table
			SET ".$champ."=" . db_param() . " WHERE id=" . db_param();
	db_query_bound( $query, Array( $lechamp, $t_bugnote_text_id ) );
	...
	$t_user_id = auth_get_current_user_id();
	$t_revision_id = bug_revision_add( $t_bug_id, $t_user_id, REV_BUGNOTE,$lechamp, $p_bugnote_id );
...
function reponse_add($f_bug_id,$f_bugnote_id,$f_date_rep,$f_reporter_id,$f_handler_id,$f_type_contact_rep,$f_accompagn,$f_aide_soc_fin,$f_aide_alim,$f_demarch,$f_sante,$f_assis_jur,$f_heberg,$f_aide_enf,$f_aide_prof,$f_autre_demarch,$f_temps,$f_rep_comm) {

	$f_bugnote_id		= db_prepare_int($f_bugnote_id);
	$f_date_rep		= db_prepare_int($f_date_rep);
	$f_reporter_id		= db_prepare_int($f_reporter_id);
	$f_handler_id	= db_prepare_int($f_handler_id);
	$f_type_contact_rep	= db_prepare_int($f_type_contact_rep);
	$f_accompagn	= db_prepare_int($f_accompagn);
	$f_aide_soc_fin	= db_prepare_string($f_aide_soc_fin);
	$f_aide_alim	= db_prepare_string($f_aide_alim);
	$f_demarch	= db_prepare_string($f_demarch);
	$f_sante	= db_prepare_string($f_sante);
	$f_assis_jur	= db_prepare_string($f_assis_jur);
	$f_heberg	= db_prepare_string($f_heberg);
	$f_aide_enf	= db_prepare_string($f_aide_enf);
	$f_aide_prof	= db_prepare_string($f_aide_prof);
	$f_autre_demarch	= db_prepare_string($f_autre_demarch);
	$f_temps	= db_prepare_int($f_temps);
	$f_rep_comm	= db_prepare_string($f_rep_comm);

	$t_reponse_table = db_get_table( 'bugnote_reponse_table' );

	# insert bugnote text
	$query = 'INSERT INTO ' . $t_reponse_table . ' ( bugnote_id,date_rep,reporter_id,handler_id,type_contact_rep,accompagn,aide_soc_fin,aide_alim,demarch,sante,assis_jur,heberg,aide_enf,aide_prof,autre_demarch,temps,rep_comm) VALUES ( ' . db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param() . ','. db_param().' )';
	
	db_query_bound( $query, Array( $f_bugnote_id,$f_date_rep,$f_reporter_id,$f_handler_id,$f_type_contact_rep,$f_accompagn,$f_aide_soc_fin,$f_aide_alim,$f_demarch,$f_sante,$f_assis_jur,$f_heberg,$f_aide_enf,$f_aide_prof,$f_autre_demarch,$f_temps,$f_rep_comm ) );

	# retrieve bugnote text id number
	$t_reponse_id = db_insert_id( $t_reponse_table );


	return $t_reponse_id;
}

function reponse_update( $reponse_id, $champ , $t_bug_id ) {

$lechamp = $GLOBALS['f_'.$champ];
$t_old_field = Resp_get( $reponse_id, $champ );
if ($lechamp!=null){
	...
	# insert an 'original' revision if needed
	/*if ( bug_revision_count( $t_bug_id, REV_BUGNOTE, $reponse_id ) < 1 ) {
		$t_user_id = bugnote_get_field( $reponse_id, 'reporter_id' );
		$t_timestamp = bugnote_get_field( $reponse_id, 'last_modified' );
		bug_revision_add( $t_bug_id, $t_user_id, REV_BUGNOTE, $t_old_field, $reponse_id, $t_timestamp );
	}*/

	$query = "UPDATE $t_reponse_table
			SET ".$champ."=" . db_param() . " WHERE id=" . db_param();
	db_query_bound( $query, Array( $lechamp, $reponse_id ) );

	# updated the last_updated date
	/*bugnote_date_update( $reponse_id );*/
	bug_update_date( $t_bug_id );

	# insert a new revision
	/*$t_user_id = auth_get_current_user_id();
	$t_revision_id = bug_revision_add( $t_bug_id, $t_user_id, REV_BUGNOTE,$lechamp, $reponse_id );*/

	# log new bugnote
	/*history_log_event_special( $t_bug_id, BUGNOTE_UPDATED, bugnote_format_id( Resp_get('bugnote_id', $reponse_id) ), $t_revision_id );*/
