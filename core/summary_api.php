		...
		if(( $row[$p_enum] != $t_last_value ) && ( -1 != $t_last_value ) ) {
			...
			switch( $p_enum ) {
				case 'status':
					...
					break;
				case 'sexe':
					$t_bug_link = '<a class="subtle" href="' . $t_filter_prefix . '&amp;' . FILTER_PROPERTY_sexe_ID . '=' . $t_last_value;
					break;
				case 'resolution':
					$t_bug_link = '<a class="subtle" href="' . $t_filter_prefix . '&amp;' . FILTER_PROPERTY_RESOLUTION_ID . '=' . $t_last_value;
					break;
				case 'motif_contact':
					$t_bug_link = '<a class="subtle" href="' . $t_filter_prefix . '&amp;' . FILTER_PROPERTY_motif_contact_ID . '=' . $t_last_value;
					break;
			}
...
function summary_print_by_age() {
	...
	$query = "SELECT * FROM $t_mantis_bug_table
				...
				AND $specific_where
				ORDER BY date_submitted ASC, motif_contact DESC";
	$result = db_query( $query );
...
function summary_print_reporter_effectiveness( $p_sexe_enum_string, $p_resolution_enum_string ) {
	$t_mantis_bug_table = db_get_table( 'mantis_bug_table' );
	$t_mantis_user_table = db_get_table( 'mantis_user_table' );
	$t_reporter_summary_limit = config_get( 'reporter_summary_limit' );
	...
	$t_sexe_multipliers = config_get( 'sexe_multipliers' );
	$t_resolution_multipliers = config_get( 'resolution_multipliers' );

	# Get the sexe values to use
	$c_sev_s = MantisEnum::getValues( $p_sexe_enum_string );
	...
	# Get all of the bugs and split them up into an array
	$query = "SELECT COUNT(id) as bugcount, reporter_id, resolution, sexe
				FROM $t_mantis_bug_table
				WHERE $specific_where
				GROUP BY reporter_id, resolution, sexe";
	$result = db_query_bound( $query );
	...
	while( $t_arr ) {
		...
		if( !isset( $t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']] ) ) {
			$t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']] = array();
			$t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']]['total'] = 0;
		}
		if( !isset( $t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']][$t_arr['resolution']] ) ) {
			$t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']][$t_arr['resolution']] = 0;
		}
		$t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']][$t_arr['resolution']] += $t_arr['bugcount'];
		$t_reporter_ressev_arr[$t_arr['reporter_id']][$t_arr['sexe']]['total'] += $t_arr['bugcount'];
		$t_reporter_bugcount_arr[$t_arr['reporter_id']] += $t_arr['bugcount'];
	...
	# value of each resolution and sexe for each user
	foreach( $t_reporter_bugcount_arr as $t_reporter_id => $t_total_user_bugs ) {
		...
		if( $t_total_user_bugs > 0 ) {
			...
			for( $j = 0; $j < $enum_sev_count; $j++ ) {
				...
				$t_sev_mult = 1;
				if( $t_sexe_multipliers[$c_sev_s[$j]] ) {
					$t_sev_mult = $t_sexe_multipliers[$c_sev_s[$j]];
				}

				if( $sev_bug_count > 0 ) {
					$t_total_sexe += ( $sev_bug_count * $t_sev_mult );
				}
			...
			echo '</td>';
			echo '<td>';
			print( $t_total_sexe - $t_total_errors );
			echo '</td>';
			echo '</tr>';
		}
	}
