...
function html_head_javascript() {
	if( ON == config_get( 'use_javascript' ) ) {
		...
		html_javascript_link( 'jquery-1.9.1.js' );
		html_javascript_link( 'jquery-ui-1.10.3.custom.js' );
		html_javascript_link( 'demo.js' );
...
function html_top_banner() {
	...
	if( !is_blank( $t_page ) && file_exists( $t_page ) && !is_dir( $t_page ) ) {
		...
		echo '<div class="logo"><h3>';
		echo '<img border="0" alt="ufm-eeia" src="' . helper_mantis_url( $t_logo_image ) . '" />';
		if( $t_show_url ) {
			echo '<a href="', config_get( 'logo_url' ), '">&nbsp;Gestion de dossiers UFM</a>';
		}
...
if ($p_file=="jeff"){
	...
	if( ON == config_get( 'show_version' ) ) {
		...
		$t_mantis_href = '<a href="/" title="Free Web-Based Bug Tracker"';
		echo
			"\t", '<span class="timer">',
	...
	if ( !is_page_name( 'login_page' ) ) {
		...
		echo '<a href="" title="Free Web Based Bug Tracker"><img src="' . helper_mantis_url( 'images/mantis_logo.png' ) . '" width="145" height="50" alt="Powered by Mantis Bugtracker" border="0" /></a>';
		echo '</div>', "\n";
	}
...
function html_body_end() {
	...
	echo '</div></div><img id="bottom" src="images/bottom.png" alt=""><a name="bottom"></a></body>', "\n";
}
...
function print_menu() {
	if( auth_is_user_authenticated() ) {
		...
		/*if( OFF == $t_protected ) {
			$t_menu_options[] = '<li><a href="' . helper_mantis_url( 'account_page.php">' ) . lang_get( 'account_link' ) . '</a></li>';
		}*/
		...
		echo '</li></ul></div><img id="top" src="images/top.png" alt=""><div id="form_container">';
	}
}
...
function print_manage_menu( $p_page = '' ) {
	...
	echo '<div class="form_description"><h2>';
	if( access_has_global_level( config_get( 'manage_user_threshold' ) ) ) {
...
function html_button( $p_action, $p_button_text, $p_fields = null, $p_method = 'post' ) {
	...
	echo "<li><form method=\"$t_method\" action=\"$p_action\">\n";
...
function html_button_bug_monitor( $p_bug_id ) {
	if( access_has_bug_level( config_get( 'monitor_bug_threshold' ), $p_bug_id ) ) {
		html_button( 'bug_monitor_add.php', '&Agrave; rappeler', array( 'bug_id' => $p_bug_id ) );
	}
}
...
class htm{
protected $p_bug_id,$resolved, $closed, $status, $readonly,$sticky;
public function lbtn( $p_bug_id ) {
	...
	echo '<div class="html_buttons"><ul>';	
function a(){
	if( !$t_readonly ) {
		# UPDATE button
		echo '<li>';
