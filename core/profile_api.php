 ...
 * @return int
 */
function profile_create( $p_user_id, $p_nom, $p_nomepouse, $p_prenom, $p_telephone, $p_description ) {
	...
	# nom cannot be blank
	if( is_blank( $p_nomepouse ) ) {
		error_parameters( lang_get( 'nomepouse' ) );
		trigger_error( ERROR_EMPTY_FIELD, ERROR );
	}

	# prenom cannot be blank
	if( is_blank( $p_prenom ) ) {
		error_parameters( lang_get( 'prenom' ) );
		trigger_error( ERROR_EMPTY_FIELD, ERROR );
	}

	# telephone cannot be blank
	if( is_blank( $p_telephone ) ) {
	...
	$query = "INSERT INTO $t_user_profile_table
				    ( user_id, nom, nomepouse, prenom, telephone, description )
				  VALUES
				    ( " . db_param() . ', '. db_param() .', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ', ' . db_param() . ' )';
	db_query_bound( $query, Array( $p_user_id, $p_nom, $nomepouse, $p_prenom, $p_telephone, $p_description ) );
 ...
 * @return true
 */
function profile_update( $p_user_id, $p_profile_id, $p_nom, $p_nomepouse, $p_prenom, $p_telephone, $p_description ) {
	$c_user_id = db_prepare_int( $p_user_id );
	$c_profile_id = db_prepare_int( $p_profile_id );
	...
	$query = "UPDATE $t_user_profile_table
				  ...
				  WHERE id=" . db_param() . " AND user_id=" . db_param();
	$result = db_query_bound( $query, Array( $p_nom,$p_nomepouse, $p_prenom, $p_telephone, $p_description, $c_profile_id, $c_user_id ) );
...
function profile_get_all_rows( $p_user_id, $p_all_users = false ) {
	...
	$query = "SELECT *
				  FROM $t_user_profile_table
				  WHERE $query_where
				  ORDER BY nom, nomepouse, prenom, telephone";
...
function profile_get_all_for_project( $p_project_id ) {
	...
	$query = "SELECT DISTINCT(up.id), up.user_id, up.nom, up.nomepouse, up.prenom, up.telephone
