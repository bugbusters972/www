...
function filter_get_url( $p_custom_filter ) {
	...
	if( !filter_field_is_any( $p_custom_filter[FILTER_PROPERTY_sexe_ID] ) ) {
		$t_query[] = filter_encode_field_and_value( FILTER_SEARCH_sexe_ID, $p_custom_filter[FILTER_PROPERTY_sexe_ID] );
	}
	...
	if( !filter_field_is_any( $p_custom_filter[FILTER_PROPERTY_motif_contact_ID] ) ) {
		$t_query[] = filter_encode_field_and_value( FILTER_SEARCH_motif_contact_ID, $p_custom_filter[FILTER_PROPERTY_motif_contact_ID] );
	}
	...
	if( !filter_field_is_any( $p_custom_filter[FILTER_PROPERTY_nom] ) ) {
		$t_query[] = filter_encode_field_and_value( FILTER_SEARCH_nom, $p_custom_filter[FILTER_PROPERTY_nom] );
	}
	if( !filter_field_is_any( $p_custom_filter[FILTER_PROPERTY_nomepouse] ) ) {
		$t_query[] = filter_encode_field_and_value( FILTER_SEARCH_nomepouse, $p_custom_filter[FILTER_PROPERTY_nomepouse] );
	}

	if( !filter_field_is_any( $p_custom_filter[FILTER_PROPERTY_prenom] ) ) {
		$t_query[] = filter_encode_field_and_value( FILTER_SEARCH_prenom, $p_custom_filter[FILTER_PROPERTY_prenom] );
	}

	if( !filter_field_is_any( $p_custom_filter[FILTER_PROPERTY_telephone] ) ) {
		$t_query[] = filter_encode_field_and_value( FILTER_SEARCH_telephone, $p_custom_filter[FILTER_PROPERTY_telephone] );
	}
...
function filter_ensure_valid_filter( $p_filter_arr ) {
	...
	if( !isset( $p_filter_arr[FILTER_PROPERTY_nom] ) ) {
		$p_filter_arr[FILTER_PROPERTY_nom] = array(
			0 => META_FILTER_ANY,
		);
	}
	if( !isset( $p_filter_arr[FILTER_PROPERTY_nomepouse] ) ) {
		$p_filter_arr[FILTER_PROPERTY_nomepouse] = array(
			0 => META_FILTER_ANY,
		);
	}

	if( !isset( $p_filter_arr[FILTER_PROPERTY_prenom] ) ) {
		$p_filter_arr[FILTER_PROPERTY_prenom] = array(
			0 => META_FILTER_ANY,
		);
	}

	if( !isset( $p_filter_arr[FILTER_PROPERTY_telephone] ) ) {
		$p_filter_arr[FILTER_PROPERTY_telephone] = array(
			0 => META_FILTER_ANY,
		);
	}
	...
	$t_multi_select_list = array(
		FILTER_PROPERTY_nom => 'string',
		FILTER_PROPERTY_nomepouse => 'string',
		FILTER_PROPERTY_prenom => 'string',
		FILTER_PROPERTY_REPORTER_ID => 'int',
		FILTER_PROPERTY_HANDLER_ID => 'int',
		FILTER_PROPERTY_NOTE_USER_ID => 'int',
		FILTER_PROPERTY_motif_nvvenue => 'int',
		FILTER_PROPERTY_motif_contact_ID => 'int',
...
function filter_get_bug_rows( &$p_page_number, &$p_per_page, &$p_page_count, &$p_bug_count, $p_custom_filter = null, $p_project_id = null, $p_user_id = null, $p_show_sticky = null ) {
	...
	# sexe
	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_sexe_ID] ) ) {
		$t_clauses = array();

		foreach( $t_filter[FILTER_PROPERTY_sexe_ID] as $t_filter_member ) {
			$c_show_sexe = db_prepare_int( $t_filter_member );
			array_push( $t_clauses, $c_show_sexe );
		}
		if( 1 < count( $t_clauses ) ) {
			$t_where_tmp = array();
			foreach( $t_clauses as $t_clause ) {
			...
			array_push( $t_where_clauses, "( $t_bug_table.sexe in (" . implode( ', ', $t_where_tmp ) . ") )" );
		} else {
			$t_where_params[] = $t_clauses[0];
			array_push( $t_where_clauses, "( $t_bug_table.sexe=" . db_param() . " )" );
		}
	}
	...
	# motif_contact
	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_motif_contact_ID] ) ) {
		$t_clauses = array();

		foreach( $t_filter[FILTER_PROPERTY_motif_contact_ID] as $t_filter_member ) {
			$c_show_motif_contact = db_prepare_int( $t_filter_member );
			array_push( $t_clauses, $c_show_motif_contact );
		}
		if( 1 < count( $t_clauses ) ) {
			$t_where_tmp = array();
			foreach( $t_clauses as $t_clause ) {
			...
			array_push( $t_where_clauses, "( $t_bug_table.motif_contact in (" . implode( ', ', $t_where_tmp ) . ") )" );
		} else {
			$t_where_params[] = $t_clauses[0];
			array_push( $t_where_clauses, "( $t_bug_table.motif_contact=" . db_param() . " )" );
		}
	}
	...
	# nom
	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_nom] ) ) {
		$t_clauses = array();

		foreach( $t_filter[FILTER_PROPERTY_nom] as $t_filter_member ) {
			$t_filter_member = stripslashes( $t_filter_member );
			if( filter_field_is_none( $t_filter_member ) ) {
				array_push( $t_clauses, '' );
			} else {
				$c_nom = db_prepare_string( $t_filter_member );
				array_push( $t_clauses, $c_nom );
			}
		}
		...
		if( 1 < count( $t_clauses ) ) {
			...
			array_push( $t_where_clauses, "( $t_bug_table.nom in (" . implode( ', ', $t_where_tmp ) . ") )" );
		} else {
			$t_where_params[] = $t_clauses[0];
			array_push( $t_where_clauses, "( $t_bug_table.nom = " . db_param() . " )" );
		}
	}

	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_nomepouse] ) ) {
		$t_clauses = array();

		foreach( $t_filter[FILTER_PROPERTY_nomepouse] as $t_filter_member ) {
			$t_filter_member = stripslashes( $t_filter_member );
			if( filter_field_is_none( $t_filter_member ) ) {
				array_push( $t_clauses, '' );
			} else {
				$c_nomepouse = db_prepare_string( $t_filter_member );
				array_push( $t_clauses, $c_nomepouse );
			}
		}
		...
		if( 1 < count( $t_clauses ) ) {
			...
			array_push( $t_where_clauses, "( $t_bug_table.nomepouse in (" . implode( ', ', $t_where_tmp ) . ") )" );
		} else {
			$t_where_params[] = $t_clauses[0];
			array_push( $t_where_clauses, "( $t_bug_table.nomepouse = " . db_param() . " )" );
		}
	}

	# os
	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_prenom] ) ) {
		$t_clauses = array();

		foreach( $t_filter[FILTER_PROPERTY_prenom] as $t_filter_member ) {
			$t_filter_member = stripslashes( $t_filter_member );
			if( filter_field_is_none( $t_filter_member ) ) {
				array_push( $t_clauses, '' );
			} else {
				$c_prenom = db_prepare_string( $t_filter_member );
				array_push( $t_clauses, $c_prenom );
			}
		}
	...
	# telephone
	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_telephone] ) ) {
		$t_clauses = array();

		foreach( $t_filter[FILTER_PROPERTY_telephone] as $t_filter_member ) {
			$t_filter_member = stripslashes( $t_filter_member );
			if( filter_field_is_none( $t_filter_member ) ) {
				array_push( $t_clauses, '' );
			} else {
				$c_telephone = db_prepare_string( $t_filter_member );
				array_push( $t_clauses, $c_telephone );
			}
		}
		...
		if( 1 < count( $t_clauses ) ) {
			...
			array_push( $t_where_clauses, "( $t_bug_table.telephone in (" . implode( ', ', $t_where_tmp ) . ") )" );
		} else {
			$t_where_params[] = $t_clauses[0];
			array_push( $t_where_clauses, "( $t_bug_table.telephone = " . db_param() . " )" );
		}
	}
	...
	if( !is_blank( $t_filter[FILTER_PROPERTY_FREE_TEXT] ) ) {
		...
		foreach( $t_search_terms as $t_search_term => $t_negate ) {
			...
			$t_textsearch_where_clause .= '( ' . db_helper_like( 'summary' ) .
				...
				' OR ' . db_helper_like( "$t_bug_text_table.motif_nvvenue" ) .
				' OR ' . db_helper_like( "$t_bug_text_table.date_nvvenue" ) .
...
function filter_draw_selection_area2( $p_page_number, $p_for_screen = true, $p_expanded = true ) {
	...
	if( $p_expanded ) {
		...
		<tr <?php echo "class=\"" . $t_trclass . "\"";?>>
			...
			<!--<td class="small-caption" valign="top">
				<a href="<?php echo $t_filters_url . FILTER_PROPERTY_sexe_ID . '[]';?>" id="show_sexe_filter"><?php echo lang_get( 'sexe' )?>:</a>
			</td>-->
			<!--<td class="small-caption" valign="top">
		...
		?>
			...
			<!--<td class="small-caption" valign="top" id="show_sexe_filter_target">
							<?php
								$t_output = '';
		$t_any_found = false;
		if( count( $t_filter[FILTER_PROPERTY_sexe_ID] ) == 0 ) {
			echo lang_get( 'any' );
		} else {
			$t_first_flag = true;
			foreach( $t_filter[FILTER_PROPERTY_sexe_ID] as $t_current ) {
				echo '<input type="hidden" name="', FILTER_PROPERTY_sexe_ID, '[]" value="', string_attribute( $t_current ), '" />';
				$t_this_string = '';
				if( filter_field_is_any( $t_current ) ) {
					$t_any_found = true;
				} else {
					$t_this_string = get_enum_element( 'sexe', $t_current );
				}
				if( $t_first_flag != true ) {
					$t_output = $t_output . '<br />';
				} else {
		...
		if( count( $t_filter['show_profile'] ) == 0 ) {
			...
			foreach( $t_filter['show_profile'] as $t_current ) {
				...
				if( filter_field_is_any( $t_current ) ) {
					...
					$t_this_string = "${t_profile['nom']} ${t_profile['nomepouse']} ${t_profile['prenom']} ${t_profile['telephone']}";
				}
				if( $t_first_flag != true ) {
					$t_output = $t_output . '<br />';
				} else {
		...
		<!--<tr <?php /*echo "class=\"" . $t_trclass . "\"";?>>
			...
			<td colspan="1" class="small-caption" valign="top">
				<a href="<?php echo $t_filters_url . FILTER_PROPERTY_motif_contact_ID . '[]';?>" id="show_motif_contact_filter"><?php echo lang_get( 'motif_contact' )?>:</a>
			</td>
			<?php if( $t_filter_cols > 8 ) {
		...
		?>
			...
			<!--<td colspan="1" class="small-caption" valign="top" id="show_motif_contact_filter_target">
              <?php
							  $t_output = '';
		$t_any_found = false;
		if( count( $t_filter[FILTER_PROPERTY_motif_contact_ID] ) == 0 ) {
			echo lang_get( 'any' );
		} else {
			$t_first_flag = true;
			foreach( $t_filter[FILTER_PROPERTY_motif_contact_ID] as $t_current ) {
				echo '<input type="hidden" name="', FILTER_PROPERTY_motif_contact_ID, '[]" value="', string_attribute( $t_current ), '" />';
				$t_this_string = '';
				if( filter_field_is_any( $t_current ) ) {
					$t_any_found = true;
				} else {
					$t_this_string = get_enum_element( 'motif_nvvenue', $t_current );
				}
				if( $t_first_flag != true ) {
					$t_output = $t_output . '<br />';
				} else {
...
<!--<td class="small-caption" valign="top" id="sticky_issues_filter_target">
		...
		<tr <?php echo "class=\"" . $t_trclass . "\"";?>>
			<td class="small-caption" valign="top">
				<?php if( ON == config_get( 'enable_profiles' ) ) { ?>
					<a href="<?php echo $t_filters_url . FILTER_PROPERTY_nom;?>" id="nom_filter"><?php echo lang_get( 'nom' )?>:</a>
				<?php } ?>
			</td>
			<td class="small-caption" valign="top">
				<?php if( ON == config_get( 'enable_profiles' ) ) { ?>
					<a href="<?php echo $t_filters_url . FILTER_PROPERTY_nomepouse;?>" id="nomepouse_filter"><?php echo lang_get( 'nomepouse' )?>:</a>
				<?php } ?>
			</td>
			<td class="small-caption" valign="top">
				<?php if( ON == config_get( 'enable_profiles' ) ) { ?>
					<a href="<?php echo $t_filters_url . FILTER_PROPERTY_prenom;?>" id="prenom_filter"><?php echo lang_get( 'prenom' )?>:</a>
				<?php } ?>
			</td>
			<!-- <td class="small-caption" valign="top">
				<?php if( ON == config_get( 'enable_profiles' ) ) { ?>
					<a href="<?php echo $t_filters_url . FILTER_PROPERTY_telephone;?>" id="telephone_filter"><?php echo lang_get( 'telephone' )?>:</a>
				<?php } ?>
			</td> -->
		...
		<tr class="row-1">
			<?php if( ON == config_get( 'enable_profiles' ) ) { ?>
			<td class="small-caption" valign="top" id="nom_filter_target">
				<?php
					print_multivalue_field( FILTER_PROPERTY_nom, $t_filter[FILTER_PROPERTY_nom] );
		?>
			</td>
			<td class="small-caption" valign="top" id="nomepouse_filter_target">
				<?php
					print_multivalue_field( FILTER_PROPERTY_nomepouse, $t_filter[FILTER_PROPERTY_nomepouse] );
		?>
			</td>
			<td class="small-caption" valign="top" id="prenom_filter_target">
				<?php
					print_multivalue_field( FILTER_PROPERTY_prenom, $t_filter[FILTER_PROPERTY_prenom] );
		?>
			</td>
			<!-- <td class="small-caption" valign="top" id="telephone_filter_target">
				<?php
					print_multivalue_field( FILTER_PROPERTY_telephone, $t_filter[FILTER_PROPERTY_telephone] );
		?>
			</td> -->
			<?php } else {?>
				<td colspan="3">&#160;</td>
...
<?php
	...
	?>
			...
			<!-- <td class="center" colspan="<?php echo( $t_filter_cols - 6 )?>"> 
				<?php
 ...
 *  print the nom field
 */
function print_filter_nom() {
	global $t_select_modifier, $t_filter;

	?>
		<!-- nom -->
		<select <?php echo $t_select_modifier;?> name="<?php echo FILTER_PROPERTY_nom;?>[]">
			<option value="<?php echo META_FILTER_ANY?>" <?php check_selected( $t_filter[FILTER_PROPERTY_nom], META_FILTER_ANY );?>>[<?php echo lang_get( 'any' )?>]</option>
			<?php
				log_event( LOG_FILTERING, 'nom = ' . var_export( $t_filter[FILTER_PROPERTY_nom], true ) );
	print_nom_option_list( $t_filter[FILTER_PROPERTY_nom] );
	?>
		</select>
		<?php
}
...
/**
 *  print the nom _naissance field
 */
function print_filter_nomepouse() {
	global $t_select_modifier, $t_filter;

	?>
		<!-- nom -->
		<select <?php echo $t_select_modifier;?> name="<?php echo FILTER_PROPERTY_nomepouse;?>[]">
			<option value="<?php echo META_FILTER_ANY?>" <?php check_selected( $t_filter[FILTER_PROPERTY_nomepouse], META_FILTER_ANY );?>>[<?php echo lang_get( 'any' )?>]</option>
			<?php
				log_event( LOG_FILTERING, 'nomepouse = ' . var_export( $t_filter[FILTER_PROPERTY_nomepouse], true ) );
	print_nomepouse_option_list( $t_filter[FILTER_PROPERTY_nomepouse] );
	?>
		</select>
		<?php
}
 ...
 *  print the os field
 */
function print_filter_prenom() {
	global $t_select_modifier, $t_filter;

	?>
		<!-- OS -->
		<select <?php echo $t_select_modifier;?> name="<?php echo FILTER_PROPERTY_prenom;?>[]">
			<option value="<?php echo META_FILTER_ANY?>" <?php check_selected( $t_filter[FILTER_PROPERTY_prenom], META_FILTER_ANY );?>>[<?php echo lang_get( 'any' )?>]</option>
			<?php print_prenom_option_list( $t_filter[FILTER_PROPERTY_prenom] )?>
		</select>
		<?php
}
 ...
 */
function print_filter_telephone() {
	global $t_select_modifier, $t_filter;

	?>
		<!-- OS Build -->
		<select <?php echo $t_select_modifier;?> name="<?php echo FILTER_PROPERTY_telephone;?>[]">
			<option value="<?php echo META_FILTER_ANY?>" <?php check_selected( $t_filter[FILTER_PROPERTY_telephone], META_FILTER_ANY );?>>[<?php echo lang_get( 'any' )?>]</option>
			<?php print_telephone_option_list( $t_filter[FILTER_PROPERTY_telephone] )?>
		</select>
		<?php
}

/**
 *  print the sexe field
 */
function print_filter_show_sexe() {
	global $t_select_modifier, $t_filter;
	?><!-- sexe -->
			<select <?php echo $t_select_modifier;?> name="<?php echo FILTER_PROPERTY_sexe_ID;?>[]">
				<option value="<?php echo META_FILTER_ANY?>" <?php check_selected( $t_filter[FILTER_PROPERTY_sexe_ID], META_FILTER_ANY );?>>[<?php echo lang_get( 'any' )?>]</option>
				<?php print_enum_string_option_list( 'sexe', $t_filter[FILTER_PROPERTY_sexe_ID] )?>
			</select>
		<?php
}
...
/**
 *  print motif_contact field
 */
function print_filter_show_motif_contact() {
	global $t_select_modifier, $t_filter;
	?><!-- motif_contact -->
    <select <?php echo $t_select_modifier;?> name="<?php echo FILTER_PROPERTY_motif_contact_ID;?>[]">
			<option value="<?php echo META_FILTER_ANY?>" <?php check_selected( $t_filter[FILTER_PROPERTY_motif_contact_ID], META_FILTER_ANY );?>>[<?php echo lang_get( 'any' )?>]</option>
			<?php print_enum_string_option_list( 'motif_contact', $t_filter[FILTER_PROPERTY_motif_contact_ID] )?>
    </select>
		<?php
}
