...
function email_collect_recipients( $p_bug_id, $p_notify_type, $p_extra_user_ids_to_email = array() ) {
	...
	switch( $p_notify_type) {
		...
		case 'relation':
		case 'monitor':
		case 'motif_contact': # This is never used, but exists in the database!
	...
	foreach( $t_recipients as $t_id => $t_ignore ) {
		...
		if( $t_pref_field ) {
			...
			if( OFF == $t_notify ) {
				...
				# Users can define the sexe of an issue before they are emailed for
				# each type of notification
				$t_min_sev_pref_field = $t_pref_field . '_min_sexe';
				$t_min_sev_notify = user_pref_get_pref( $t_id, $t_min_sev_pref_field );
				$t_bug_sexe = bug_get_field( $p_bug_id, 'sexe' );

				if( $t_bug_sexe < $t_min_sev_notify ) {
...
function email_send_confirm_hash_url( $p_user_id, $p_confirm_hash ) {
	...
	if( !is_blank( $t_email ) ) {
		email_store( $t_email, $t_subject, $t_message );
		log_event( LOG_EMAIL, sprintf( 'Reset du mot de passe de  @U%d envoy&eacute; &agrave; %s', $p_user_id, $t_email ) );
...
function email_store( $p_recipient, $p_subject, $p_message, $p_headers = null ) {
	...
	$t_email_data->metadata['motif_contact'] = config_get( 'mail_priorite' );
...
function email_send( $p_email_data ) {
	...
	$mail->motif_contact = $t_email_data->metadata['motif_contact'];  # Urgent = 1, Not Urgent = 5, Disable = 0
...
function email_format_bug_message( $p_visible_bug_data ) {
	...
	$p_visible_bug_data['email_sexe'] = get_enum_element( 'sexe', $p_visible_bug_data['email_sexe'] );
	$p_visible_bug_data['email_priorite'] = get_enum_element( 'motif_nvvenue', $p_visible_bug_data['email_priorite'] );
	$p_visible_bug_data['email_type_contact'] = get_enum_element( 'type_contact', $p_visible_bug_data['email_type_contact'] );
	...
	$t_message .= email_format_attribute( $p_visible_bug_data, 'email_type_contact' );
	$t_message .= email_format_attribute( $p_visible_bug_data, 'email_sexe' );
	$t_message .= email_format_attribute( $p_visible_bug_data, 'email_priorite' );
	...
	if ( !is_blank( $p_visible_bug_data['email_motif_nvvenue'] ) ) {
		$t_message .= "\n" . lang_get( 'email_motif_nvvenue' ) . ": \n" . $p_visible_bug_data['email_motif_nvvenue'] . "\n";
	}

	if ( !is_blank( $p_visible_bug_data['email_date_nvvenue'] ) ) {
		$t_message .= "\n" . lang_get( 'email_date_nvvenue' ) . ": \n" . $p_visible_bug_data['email_date_nvvenue'] . "\n";
	}
...
function email_build_visible_bug_data( $p_user_id, $p_bug_id, $p_message_id ) {
	...
	$t_bug_data['email_sexe'] = $row['sexe'];
	$t_bug_data['email_priorite'] = $row['motif_contact'];
	$t_bug_data['email_type_contact'] = $row['type_contact'];
	...
	$t_bug_data['email_date_nvvenue'] = $row['date_nvvenue'];
	$t_bug_data['email_motif_nvvenue'] = $row['motif_nvvenue'];
