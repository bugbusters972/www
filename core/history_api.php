...
function history_localize_item( $p_field_name, $p_type, $p_old_value, $p_new_value, $p_linkify=true ) {
	...
	switch( $p_field_name ) {
		...
		case 'status':
			...
			break;
		case 'sexe':
			$p_old_value = get_enum_element( 'sexe', $p_old_value );
			$p_new_value = get_enum_element( 'sexe', $p_new_value );
			$t_field_localized = lang_get( 'sexe' );
			break;
		case 'type_contact':
			$p_old_value = get_enum_element( 'type_contact', $p_old_value );
			$p_new_value = get_enum_element( 'type_contact', $p_new_value );
			$t_field_localized = lang_get( 'type_contact' );
			break;
		case 'resolution':
			...
			break;
		case 'motif_nvvenue':
			$p_old_value = get_enum_element( 'motif_nvvenue', $p_old_value );
			$p_new_value = get_enum_element( 'motif_nvvenue', $p_new_value );
			$t_field_localized = lang_get( 'motif_nvvenue' );
			break;
		case 'eta':
		...
		case 'last_updated':
			...
			break;
		case 'prenom':
			$t_field_localized = lang_get( 'prenom' );
			break;
		case 'telephone':
			$t_field_localized = lang_get( 'telephone' );
			break;
		case 'build':
			$t_field_localized = lang_get( 'build' );
			break;
		case 'nom':
			$t_field_localized = lang_get( 'nom' );
			break;
			case 'nomepouse':
			$t_field_localized = lang_get( 'nomepouse' );
			break;
		case 'summary':
		...
		if( NORMAL_TYPE != $p_type ) {
			switch( $p_type ) {
				...
				case DESCRIPTION_UPDATED:
					...
					break;
				case DATE_NVVENUE_UPDATED:
					$t_note = lang_get( 'date_nvvenue_updated' );
					$t_old_value = (int)$p_old_value;
					if ( $p_linkify && bug_revision_exists( $t_old_value ) ) {
						$t_change = '<a href="bug_revision_view_page.php?rev_id=' . $t_old_value . '#r' . $t_old_value . '">' .
							lang_get( 'view_revisions' ) . '</a>';
				...
				case STEP_TO_REPRODUCE_UPDATED:
					$t_note = lang_get( 'motif_nvvenue_updated' );
					$t_old_value = (int)$p_old_value;
					if ( $p_linkify && bug_revision_exists( $t_old_value ) ) {
						$t_change = '<a href="bug_revision_view_page.php?rev_id=' . $t_old_value . '#r' . $t_old_value . '">' .
							lang_get( 'view_revisions' ) . '</a>';
