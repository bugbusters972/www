...
function print_tag_attach_form( $p_bug_id, $p_string = '' ) {
	...
	?>
		<input type="submit" value="<?php echo lang_get( 'tag_attach' )?>"  />
		</form>
		<?php
		return true;
}
...
function print_profile_option_list_from_profiles( $p_profiles, $p_select_id ) {
	...
	foreach( $p_profiles as $t_profile ) {
		...
		$t_nom = string_attribute( $t_profile['nom'] );
		$t_nomepouse = string_attribute( $t_profile['nomepouse'] );
		$t_prenom = string_attribute( $t_profile['prenom'] );
		$t_telephone = string_attribute( $t_profile['telephone'] );

		echo '<option value="' . $t_profile['id'] . '"';
		check_selected( $p_select_id, $t_profile['id'] );
		echo '>' . $t_nom . ' ' . $t_prenom . ' ' . $t_telephone . '</option>';
	}
}
...
# --------------------
# Print the option list for noms accessible for the specified user.
function print_nom_option_list( $p_nom, $p_user_id = null ) {
	$t_noms_array = profile_get_field_all_for_user( 'nom', $p_user_id );

	foreach( $t_noms_array as $t_nom ) {
		$t_nom = string_attribute( $t_nom );
		echo '<option value="' . $t_nom . '"';
		check_selected( string_attribute( $p_nom ), $t_nom );
		echo '>' . $t_nom . '</option>';
	}
}
...
# --------------------
# Print the option list for noms accessible for the specified user.
function print_nomepouse_option_list( $p_nomepouse, $p_user_id = null ) {
	$t_noms_naissance_array = profile_get_field_all_for_user( 'nomepouse', $p_user_id );

	foreach( $t_noms_naissance_array as $t_nomepouse ) {
		$t_nomepouse = string_attribute( $t_nomepouse );
		echo '<option value="' . $t_nomepouse . '"';
		check_selected( string_attribute( $p_nomepouse ), $t_nomepouse );
		echo '>' . $t_nomepouse . '</option>';
	}
}

# --------------------
# Print the option list for OSes accessible for the specified user.
function print_prenom_option_list( $p_prenom, $p_user_id = null ) {
	$t_prenom_array = profile_get_field_all_for_user( 'prenom', $p_user_id );

	foreach( $t_prenom_array as $t_prenom ) {
		$t_prenom = string_attribute( $t_prenom );
		echo '<option value="' . $t_prenom . '"';
		check_selected( string_attribute( $p_prenom ), $t_prenom );
		echo '>' . $t_prenom . '</option>';
	}
}

# Print the option list for telephone accessible for the specified user.
function print_telephone_option_list( $p_telephone, $p_user_id = null ) {
	$t_telephone_array = profile_get_field_all_for_user( 'telephone', $p_user_id );

	foreach( $t_telephone_array as $t_telephone ) {
		$t_telephone = string_attribute( $t_telephone );
		echo '<option value="' . $t_telephone . '"';
		check_selected( string_attribute( $p_telephone ), $t_telephone );
		echo '>' . $t_telephone . '</option>';
	}
}
...
# + fonction radio liste
function print_enum_string_radio_list( $p_enum_name, $p_val = 0) {
	$t_config_var_name = $p_enum_name . '_enum_string';
	$t_config_var_value = config_get( $t_config_var_name );
	...
	foreach ( $t_enum_values as $t_key ) {
		...
		echo '<input value="' . $t_key . '"';
		check_checked( $p_val, $t_key );
		echo 'name="'.$p_enum_name.'" type="radio"/><label for="'.$p_enum_name.'">' . $t_elem2 . '</label>';
	}
}

# + fonction checkbox liste
function print_enum_string_checkbox_list( $p_enum_name, $array = array()) {
	$t_config_var_name = $p_enum_name . '_enum_string';
	$t_config_var_value = config_get( $t_config_var_name );

	$t_enum_values = MantisEnum::getValues( $t_config_var_value );
	foreach ( $t_enum_values as $t_key ) {//10, 20 40 45
		$t_elem2 = get_enum_element( $p_enum_name, $t_key );// bla bla bla

		echo '<input value="' . $t_key . '"';
		if( in_array($t_key,$array)) { echo ' checked="checked" ';}
		echo ' type="checkbox" id="'.$p_enum_name.'"/><label for="'.$p_enum_name.'">' . $t_elem2 . '</label>';
	}
	echo '<input type="hidden" name="'.$p_enum_name.'" value="" />';
}
 ...
 * @param int current motif_contact
 */
function print_plugin_priorite_list( $p_motif_contact ) {
	if( $p_motif_contact < 1 && $p_motif_contact > 5 ) {
		echo '<option value="', $p_motif_contact, '" selected="selected">', $p_motif_contact, '</option>';
	}

	for( $i = 5;$i >= 1;$i-- ) {
		echo '<option value="', $i, '" ', check_selected( $p_motif_contact, $i ), ' >', $i, '</option>';
	}
}
...
# formats the motif_contact given the status
# shows the motif_contact in BOLD if the bug is NOT closed and is of significant motif_contact
function print_formatted_motif_nvvenue_string( $p_bug ) {
	$t_pri_str = get_enum_element( 'motif_nvvenue', $p_bug->motif_nvvenue, auth_get_current_user_id(), $p_bug->project_id );
	$t_motif_nvvenue_threshold = config_get( 'motif_contact_significant_threshold' );

	if( $t_motif_nvvenue_threshold >= 0 &&
		$p_bug->motif_nvvenue >= $t_motif_nvvenue_threshold &&
...
# formats the sexe given the status
# shows the sexe in BOLD if the bug is NOT closed and is of significant sexe
function print_formatted_sexe_string( $p_bug ) {
	$t_sev_str = get_enum_element( 'sexe', $p_bug->sexe, auth_get_current_user_id(), $p_bug->project_id );
	$t_sexe_threshold = config_get( 'sexe_significant_threshold' );

	if( $t_sexe_threshold >= 0 &&
		$p_bug->sexe >= $t_sexe_threshold &&
		$p_bug->status < config_get( 'bug_closed_status_threshold' ) ) {
